namespace Todo.Application.Parameters
{
    ...
    public class GetParameterQueryHandler : IRequestHandler<GetParameterQuery, GenericResponse<List<GetParameterResponse>>>
    {
        ...
        {
            var response = _unitOfWork.Parameter.Find(x => x.IsActive).Select(x => new GetParameterResponse { Name = x.Name, ParameterTypeId = x.ParameterTypeId }).ToList();
